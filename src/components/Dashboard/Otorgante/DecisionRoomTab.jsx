...
function buildDecision(opportunity, L, copy) {
  ...
  return {
    ...
    nextAction: L(`Mantener contacto controlado por ${BRAND.name} y pedir paquete documental minimo.`, `Keep contact controlled by ${BRAND.name} and request minimum document package.`),
  };
}
...
function buildGateRows(opportunity, L) {
  ...
  return [
    ...
    [L("Score minimo", "Minimum score"), score >= 65, `${score}/100 ${L(`como lectura preliminar ${BRAND.name}.`, `as ${BRAND.name} preliminary read.`)}`],
