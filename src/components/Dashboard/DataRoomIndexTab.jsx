...
export default function DataRoomIndexTab() {
  ...
  const folders = [
    ...
    ["01", L("Identidad y KYB", "Identity & KYB"), L("RFC, poderes, beneficiario controlador, identificaciones", "Tax ID, powers of attorney, beneficial owner, identifications"), "Completo", L(`${BRAND.name} + Otorgantes`, `${BRAND.name} + Funding Providers`)],
    ["02", L("Corporativo Legal", "Corporate Legal"), L("Actas, estatutos, estructura, autorizaciones", "Bylaws, articles, structure, authorizations"), "En revision", BRAND.name],
    ["03", L("Financiero", "Financial"), L("Estados financieros, flujo, deuda, ingresos", "Financial statements, cash flow, debt, revenue"), "Faltantes", L(`${BRAND.name} + Otorgantes`, `${BRAND.name} + Funding Providers`)],
    ...
    ["05", L("Cumplimiento", "Compliance"), L("Declaraciones, antifraude, PLD/KYC, listas internas", "Declarations, anti-fraud, AML/KYC, internal screening lists"), "Completo", BRAND.name],
  ...
  const infoCards = [
    ...
    [L("Permisos por rol", "Permissions by Role"), L(`El solicitante carga, ${BRAND.name} revisa y el otorgante ve solo lo autorizado.`, `The applicant uploads, ${BRAND.name} reviews, and the funding provider only sees authorized items.`)],
    ...
    <div style={{ display: "grid", gap: "1rem" }}>
      <section style={{
        ...
        <p style={{ margin: 0, maxWidth: "820px", color: "rgba(255,255,255,0.82)", lineHeight: 1.6 }}>
          {L(
            `Esta estructura ayuda a solicitantes, ${BRAND.name} y otorgantes a revisar el mismo expediente sin perder trazabilidad, permisos ni version documental.`,
            `This structure helps applicants, ${BRAND.name} and funders review the same file without losing traceability, permissions or document versioning.`
          )}
        </p>
      </section>
