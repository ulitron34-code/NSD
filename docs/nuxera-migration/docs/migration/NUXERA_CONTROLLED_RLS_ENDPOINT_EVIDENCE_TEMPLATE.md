...
**Resolved locally 2026-07-22, pending push/deploy confirmation:** the 2026-07-19/20 deployed evidence above correctly observed zero `audit_logs` rows for `GET /api/nuxera/orders/:orderId/grantor-evidence`. A later local change now emits `nuxera_grantor_evidence_read` after the authorized-grantor read succeeds, with `orderId`, requester email, link count, persistence flag and request metadata. Unauthorized/no-token attempts still stop before the evidence service and do not emit this event. This must still be pushed to PR #3, deployed, and verified with a real `audit_logs` row before the deployed evidence table can be marked audited.
