...
| Backend persistence contracts | N/A | NUXERA state contract over existing service orders/documents/audit | Applicant checklist state skeleton, route coverage and static SQL guard verified; SQL not applied | N/A |
...
| Applicant dashboard | Yes | Progress and next-action home | Guided mission with read-only NUXERA state fallback mounted | No |
