...
| Backend persistence contracts | N/A | NUXERA state contract over existing service orders/documents/audit | Applicant checklist, owner evidence-link and read-only admin control skeletons implemented; consolidated SQL guard, controlled RLS/endpoint plan guard, protected verification-plan endpoint, evidence scaffold endpoint/CLI, controlled runbook endpoint, evidence review endpoint/CLI, approval package endpoint/CLI, controlled write gate endpoint/CLI, controlled change request endpoint/CLI, release readiness dossier endpoint/CLI, continuation pack endpoint/CLI, admin evidence package surface and SQL/RLS readiness checklist local | N/A |
