...
| `POST` | `/nuxera/admin/notification-delivery-batch` | `*` or `nuxera:admin:update` | Manually runs a small controlled delivery batch. It never trusts client-supplied delivery flags; backend env gates decide whether rows are read/updated or emails are sent. No cron/scheduler is attached. |
