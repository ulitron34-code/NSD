...
| `COMMERCIAL_CATALOG_ENABLED` | `false` | Read by `backend/src/routes/commercialCatalog.js`. Gates `GET /api/commercial/offers`, the single source of truth for plan/package pricing cards (Fase 1 of the commercial model plan). When not `true`, the route returns 404 and no catalog data is exposed. It never resolves a client-supplied amount; `commercialCatalogService.js` only reads `commercial_offers`/`commercial_prices`/`offer_entitlements`. |
